...
class Coach:
    ...
    def test_epoch(self, logPredict=False):
        ...
        with t.no_grad():
            for i, batch_data in enumerate(tst_loader):
                ...
                all_ids = t.arange(1, item_emb.shape[0]).unsqueeze(0).expand(seq.shape[0], -1).cuda()
                all_emb = item_emb[all_ids] # (batch, 100, latdim)
                all_scr = t.sum(t.unsqueeze(seq_emb, 1) * all_emb, -1) # (batch, 100)
                # print(all_scr)
                sort_index, batch = self.sort(all_scr, pos, all_ids)
                # print(sort_index)
                sort_lists.append(sort_index)

        sort_lists = t.cat(sort_lists, dim=0)

        Recall5 = self._calc_Recall(sort_lists, num, 5)
        Recall10 = self._calc_Recall(sort_lists, num, 10)
        Recall50 = self._calc_Recall(sort_lists, num, 50)
        NDCG5 = self._calc_NDCG(sort_lists, num, 5)
        NDCG10 = self._calc_NDCG(sort_lists, num, 10)
        NDCG50 = self._calc_NDCG(sort_lists, num, 50)
        ...
        print(f"Test Result: R@10:{Recall10}, R@50:{Recall50}, NDCG@10:{NDCG10}, NDCG@50:{NDCG50}")
    ...
    def sort(self, predicts, labels, all_ids):
        predicts_sort = t.argsort(predicts, dim=-1, descending=True)
        diff = predicts_sort - labels + 1
