...
class Coach:
    def __init__(self, handler):
        ...
        self.text_emb = handler.text_emb.cuda()
    ...
    def train_epoch(self):
        ...
        for i, batch_data in enumerate(trn_loader):
            ...
            seq_emb = self.recommender(seq, item_emb, self.text_emb)
            tt_emb = t.cat((item_emb, self.text_emb), dim=1)
            tar_msk = pos > 0
            # print("Shape of seq_emb:", seq_emb.shape)
            # print("Shape of tt_emb[pos]:", tt_emb[pos].shape)
            # print("Shape of tt_emb[neg]:", tt_emb[neg].shape)

            loss_main = cross_entropy(seq_emb, tt_emb[pos], tt_emb[neg], tar_msk)
...
if __name__ == '__main__':
    ...
    handler.load_data()
    # log('Load Data')
    # text_emb = nn.Parameter(torch.empty(args.item, args.latdim).cuda()) # args.item = num_real_item + 1
