...
class Encoder(nn.Module):
    def __init__(self):
        ...
        self.item_emb = nn.Parameter(init(t.empty(args.item, args.latdim//2))) # args.item = num_real_item + 1
    ...
    def forward(self, encoder_adj):
        ...
        # Concatenate the second half of the items to the first half
        all_embeds = sum(embeds)
        half1, half2 = t.chunk(all_embeds, 2, dim=0)
        node_embeds = t.cat((half1, half2), dim=-1)
        return node_embeds, embeds
...
#         loss = -t.sum(pos_scr / (x     + 1e-8) + 1e-8)
#         return loss
...
class Decoder(nn.Module):
    def __init__(self):
        ...
        self.MLP = nn.Sequential(
            nn.Linear(args.latdim // 2 * args.num_gcn_layers ** 2, args.latdim * args.num_gcn_layers, bias=True),
            nn.ReLU(),
