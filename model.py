...
class SASRec(nn.Module):
    def __init__(self):
        ...
        self.pos_emb = nn.Parameter(init(t.empty(args.max_seq_len, args.latdim+512)))
        self.layers = nn.Sequential(*[TransformerLayer() for i in range(args.num_trm_layers)])
        self.LayerNorm = nn.LayerNorm(args.latdim+512)
        self.dropout = nn.Dropout(args.hidden_dropout_prob)
        self.apply(self.init_weights)

    def get_seq_emb(self, sequence, item_emb, text_emb):
        seq_len = sequence.size(1)
        pos_ids = t.arange(seq_len, dtype=t.long, device=sequence.device)
        pos_ids = pos_ids.unsqueeze(0).expand_as(sequence)
        tt_emb = t.cat((item_emb, text_emb), dim=1)
        # itm_emb = item_emb[sequence]
        # tex_emb = text_emb[sequence]
        pos_emb = self.pos_emb[pos_ids]
        t_emb = tt_emb[sequence]
        seq_emb = t_emb + pos_emb
        
        seq_emb = self.LayerNorm(seq_emb)
        seq_emb = self.dropout(seq_emb)
        return seq_emb

    def forward(self, input_ids, item_emb, text_emb):
        ...
        seq_embs = [self.get_seq_emb(input_ids, item_emb, text_emb)]
...
class SelfAttentionLayer(nn.Module):
    def __init__(self):
        ...
        self.attention_head_size = int((args.latdim+512)/ args.num_attention_heads)
        self.all_head_size = self.num_attention_heads * self.attention_head_size

        self.query = nn.Linear(args.latdim+512, self.all_head_size)
        self.key = nn.Linear(args.latdim+512, self.all_head_size)
        self.value = nn.Linear(args.latdim+512, self.all_head_size)

        self.attn_dropout = nn.Dropout(args.attention_probs_dropout_prob)

        self.dense = nn.Linear(args.latdim+512, args.latdim+512)
        self.LayerNorm = nn.LayerNorm(args.latdim+512)
...
class IntermediateLayer(nn.Module):
    def __init__(self):
        ...
        self.layers = nn.Sequential(
            nn.Linear(args.latdim+512, args.latdim * 4, bias=True),
            nn.GELU(),
            nn.Linear(args.latdim * 4, args.latdim+512, bias=True),
            nn.Dropout(args.hidden_dropout_prob),
            nn.LayerNorm(args.latdim+512)
        )
